...
/**
 * The SDK is instrumented with log() calls to record errors, warnings and
 * server traffic for diagnostic purposes. A small buffer of log events are kept
 * in a memory buffer, and events are also sent to the system log (accessible
 * with the logcat interface.) When reporting problem it is helpful to capture
 * this diagnostic information to aid with debugging and resolution.
 */
final public class MASTAdLog {
	...
	// Maximum number of messages to keep in memory for review access through
	// the app;
	// keep this small by default to conserve memory usage. When debugging apps,
	// callers
	...
	/**
	 * Set default log level to one of the log level values defined in he
	 * MASTAdLog class (corresponding to errors only, or everything including
	 * server traffic.)
	 * 
	 * @param logLevel
	 *            Int log level to set as the default value (initially NONE).
	 */
	 ...
	 * 
	 * @param adView
	 *            Base view object associated with logging
	 */
	 ...
	 * 
	 * @param Level
	 *            Int logging level, from 0 (none) to 2 (errors, warnings and
	 *            server traffic)
	 * @param tag
	 *            String log message tag
	 * @param msg
	 *            String log message detail
	 */
	public void log(int Level, String tag, String msg) {
		...
		String resultTag;
		if (adView != null) {
			resultTag = "[" + Integer.toHexString(adView.hashCode()) + "]"
					+ tag;
		} else {
		...
		if (adView != null) {
			...
			if (delegate != null) {
				MASTAdDelegate.LogEventHandler logHandler = delegate
						.getLogEventHandler();
				if (logHandler != null) {
					boolean logEvent = logHandler.onLogEvent(Level, resultTag
							+ msg);
					if (!logEvent) {
	...
	/**
	 * Set log level to one of the log level values defined in he MASTAdLog
	 * class (corresponding to errors, errors + warnings, or everything
	 * including server traffic.) The SDK is instrumented with diagnostics
	 * logging that can assist with troubleshooting integration problems. Log
	 * messages are sent to the system logging interface (viewable with logcat)
	 * and an in-memory log of recent messages is stored for easy access.
	 * 
	 * @see MASTAdLog See the MASTAdLog class for more information about
	 *      logging.
	 * @param logLevel
	 *            Int log level to control which messages will be sent to the
	 *            logs.
	 */
