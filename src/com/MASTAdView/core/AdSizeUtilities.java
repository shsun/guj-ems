...
final public class AdSizeUtilities {
	...
	synchronized public String startExpand(Bundle data, AdReloadTimer timer) {
		...
		try {
			...
			value = data
					.getString(MraidInterface
							.get_ORIENTATION_PROPERTIES_name(MraidInterface.ORIENTATION_PROPERTIES.ALLOW_ORIENTATION_CHANGE));
			...
			forceOrientation = data
					.getString(MraidInterface
							.get_ORIENTATION_PROPERTIES_name(MraidInterface.ORIENTATION_PROPERTIES.FORCE_ORIENTATION));
	...
	private void updateOrientation() {
		...
		if (expandAllowOrientationChange == true) {
			...
			if (expandForceOrientation == MraidInterface.FORCE_ORIENTATION_PROPERTIES.NONE) {
				int currentOrientation = ((Activity) context).getResources()
	...
	private String resizePropertiesValid(int toWidth, int toHeight,
			String closePosition, int offsetX, int offsetY, boolean offscreen) {
		...
		try {
			...
			if (closePosition == null) {
				closePosition = MraidInterface
						.get_RESIZE_CUSTOM_CLOSE_POSITION_name(MraidInterface.RESIZE_CUSTOM_CLOSE_POSITION.TOP_RIGHT);
			}
	...
	private String resizeWorker(int toWidth, int toHeight,
		...
		if (!allowOffScreen) {
			...
			if ((closeX < screenMinX) || (closeY < screenMinY)
					|| (closeX + closeControlSize > screenMaxX)
					|| (closeY + closeControlSize > screenMaxY)) {
		...
		// NOTE: this needs to come before the block below so that the state is
		...
		if ((parent instanceof AdViewContainer) == false) {
			// ad view already displayed in resized "overlay", simply adjust the
			// size/position
			...
			ViewGroup.LayoutParams plp = new ViewGroup.LayoutParams(
					...
					ViewGroup.LayoutParams.FILL_PARENT);
			// ViewGroup.LayoutParams plp = new ViewGroup.LayoutParams(toWidth,
			// toHeight);
			...
			// Create close button (transparent, as visual indicator is provided
			// by ad)
