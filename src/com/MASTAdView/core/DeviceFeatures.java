...
final public class DeviceFeatures {
	 ...
	 * { switch(feature) { case SMS: cacheSmsSupport = value; case PHONE:
	 * cachePhoneSupport = value; //case EMAIL: //return emailSupport(); case
	 * CALENDAR: cacheCalendarSupport = value; case STORE_PICTURE:
	 * cachePictureSupport = value; case INLINE_VIDEO: cacheVideoSupport =
	 * value; } }
	 */
	...
	public boolean smsSupport() {
		...
		if (delegate != null) {
			MASTAdDelegate.FeatureSupportHandler handler = delegate
	...
	public boolean calendarSupport() {
		...
		if (cacheCalendarSupport == null) {
			cacheCalendarSupport = ((context
					.checkCallingOrSelfPermission(android.Manifest.permission.WRITE_CALENDAR) == PackageManager.PERMISSION_GRANTED) && (context
	...
	public String createCalendarInteractive(String description,
			String location, String summary, String start, String end) {
		try {
			Intent intent = new Intent(Intent.ACTION_EDIT);
			intent.setType("vnd.android.cursor.item/event");
			intent.putExtra("dtstart", parseDateString(start)); // Events.DTSTART
																// from API 14
			intent.putExtra("dtend", parseDateString(end)); // Events.DTEND from
															// API 14
			intent.putExtra("title", summary); // Events.TITLE from API 14
			intent.putExtra("eventLocation", location); // Events.EVENT_LOCATION
														// from API 14
			intent.putExtra("descriptoin", description); // Events.DESCRIPTION
															// from API 14
	...
	synchronized private Long parseDateString(String input) {
		try {
			...
			logger.log(MASTAdLog.LOG_LEVEL_ERROR,
					"DeviceFeatures exception parsing date", ex.getMessage());
		}
	...
	public boolean inlineVideoSupport() {
		...
		if (cacheVideoSupport == null) {
			// Honeycomb and later OS versions can playback inline with hardware
			// support
			final int honeycomb = 11; // Build.VERSION_CODES.HONEYCOMB if using
										// API 3.0 or above
			if ((Build.VERSION.SDK_INT >= honeycomb)
					&& (context instanceof Activity)) {
				// inline can work, must turn on hardware acceleration for web
				// views, but can only
				// do this if our context is an activity which allows access to
				// the window properties

				// XXX true when ready, currently this causes hw acceleration to
				// be enabled in the web view, which cases some problems
	...
	public String playVideo(String videoUrl) {
		// Launch the play video intent and let android use the preferred player
		// for this device/user
		Uri uri = Uri.parse(videoUrl);
		Intent i = new Intent(Intent.ACTION_VIEW, uri);
		if (i.resolveActivity(context.getPackageManager()) == null) {
			String error = "No video playback handler found, skipping..."; // XXX
																			// string
			MASTAdLog logger = new MASTAdLog(null);
			logger.log(MASTAdLog.LOG_LEVEL_ERROR, "DeviceFeatures", error);
			return error;
