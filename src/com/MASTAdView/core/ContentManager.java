...
final public class ContentManager {
	...
	synchronized static public ContentManager getInstance(
		...
		if (senderParameters.containsKey(consumer)) {
			...
			if (cTh != null) {
				try {
					...
					logger.log(MASTAdLog.LOG_LEVEL_DEBUG, "ContentManager",
							"Error stopping thread, ignored...");
				}
			}
	...
	final private class ContentThread extends Thread {
		...
		public void run() {
			try {
				...
				/*
				 * if (useBuiltinTestAds) { String adText = null;
				 * 
				 * int random = (int)(System.currentTimeMillis() % 3); // 0 or 1
				 * or 2 if (random == 0) { // Text adText =
				 * Utils.readRawAsset(context, R.raw.test_text_xml); } else if
				 * (random == 1) { // Image adText = Utils.readRawAsset(context,
				 * R.raw.test_image_xml); } else { // Richmedia adText =
				 * Utils.readRawAsset(context, R.raw.test_richmedia_xml); }
				 * 
				 * if (adText != null) { AdParser parser = new AdParser();
				 * AdData ad = parser.parse(adText); if (parameters.sender !=
				 * null) { parameters.sender.setResult(ad); return; } } }
				 */
				...
				HttpConnectionParams.setConnectionTimeout(get.getParams(),
						MASTAdConstants.AD_RELOAD_PERIOD);
				HttpConnectionParams.setSoTimeout(get.getParams(),
						MASTAdConstants.DEFAULT_REQUEST_TIMEOUT);
				HttpResponse response = client.execute(get);

				if (response.getStatusLine().getStatusCode() != 200) {
					setErrorResult("Response code = "
	...
	/*
	 * public String getDeviceId() { return getDeviceId((TelephonyManager)
	 * context.getSystemService(Context.TELEPHONY_SERVICE)); }
	 * 
	 * // Return user-specified device ID value if any, otherwise unique device
	 * ID from // phone if that option has been enabled. synchronized private
	 * String getDeviceId(TelephonyManager tm) { if (id != null) { return id; }
	 * 
	 * String deviceId = null; if (useSystemDeviceId) { String tempDeviceId =
	 * tm.getDeviceId();
	 * 
	 * if (null != tempDeviceId) { deviceId = tempDeviceId; } else {
	 * tempDeviceId = Secure.getString(context.getContentResolver(),
	 * Secure.ANDROID_ID);
	 * 
	 * if (null != tempDeviceId) { deviceId = tempDeviceId; } else { deviceId =
	 * makeDeviceId(context); } } }
	 * 
	 * id = deviceId; return deviceId; }
	 */
	...
	private void initDefaultParameters() {
		 ...
		 * autoDetectParameters += "&"+MASTAdRequest.parameter_device_id+"=" +
		 * deviceIdMd5; }
		 */
	}

	/*
	 * private synchronized String makeDeviceId(Context context) { if (id ==
	 * null) { File installation = new File(context.getFilesDir(),
	 * INSTALLATION); try { if (!installation.exists())
	 * writeInstallationFile(installation); id =
	 * readInstallationFile(installation); } catch (Exception e) { id =
	 * "1234567890"; } }
	 * 
	 * return id; }
	 * 
	 * 
	 * synchronized public boolean getUseSystemDeviceId() { return
	 * useSystemDeviceId; }
	 * 
	 * 
	 * synchronized public void setUseSystemDeviceId(boolean value) { boolean
	 * changed = false; if (useSystemDeviceId != value) { changed = true; }
	 * useSystemDeviceId = value;
	 * 
	 * if (changed) { runInitDefaultParameters(); } }
	 * 
	 * 
	 * synchronized public void setDeviceId(String value) { boolean changed =
	 * false; if ((id != null) && (id.compareTo(value) != 0)) { changed = true;
	 * } id = value;
	 * 
	 * if (changed) { runInitDefaultParameters(); } }
	 * 
	 * private static String readInstallationFile(File installationFile) throws
	 * IOException { RandomAccessFile f = new RandomAccessFile(installationFile,
	 * "r"); byte[] bytes = new byte[(int) f.length()]; f.readFully(bytes);
	 * f.close(); return new String(bytes); }
	 * 
	 * private static void writeInstallationFile(File installationFile) throws
	 * IOException { FileOutputStream out = new
	 * FileOutputStream(installationFile); String id =
	 * UUID.randomUUID().toString(); out.write(id.getBytes()); out.close(); }
	 */
}
