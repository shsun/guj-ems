...
public class AdViewContainer extends RelativeLayout implements
		ContentManager.ContentConsumer {
	...
	private void initialize(Context c) {
		...
		WindowManager windowManager = (WindowManager) ((Activity) context)
		...
		adImageView = createImageView(context, true); // create view with gif
														// animation support
	...
	private TextView createTextView(Context context) {
		...
		// parent level
		v.setLayoutParams(new ViewGroup.LayoutParams(LayoutParams.FILL_PARENT,
	...
	private View createImageView(Context context,
			boolean withAnimataedGifSupport) {
		View v;
	...
	public AdWebView createWebView(final Context context) {
		...
		boolean handleClicks = true;
		AdWebView v = new AdWebView(this, adLog, metrics, supportMraid,
				handleClicks);
				...
				new ViewTreeObserver.OnGlobalLayoutListener() {
					public void onGlobalLayout() {
						if ((adWebView != null)
							...
							adWebView.getLocationOnScreen(coordinates); // getLocationInWindow()
																		// for
																		// relative
	...
	public String getUserAgent() {
		String userAgent = (String) adserverRequest
				.getProperty(MASTAdRequest.parameter_userAgent);
		if (userAgent == null) {
			userAgent = adWebView.getSettings().getUserAgentString();

			if ((userAgent != null) && (userAgent.length() > 0)) {
				adserverRequest.setProperty(MASTAdRequest.parameter_userAgent,
						userAgent);
			}
		}
	...
	protected void setAutomaticParameters(Context context) {
		...
		if ((version != null) && (version.length() > 0)) {
			adserverRequest.setProperty(MASTAdRequest.parameter_version,
					version);
		}
	...
	protected void onSizeChanged(int w, int h, int ow, int oh) {
		if (adWebView != null) {
			adWebView.getLocationOnScreen(coordinates); // getLocationInWindow()
														// for relative
			adWebView.getMraidInterface().setCurrentPosition(coordinates[0],
					coordinates[1], w, h);
	...
	private void setWebContent(String webData) {
		...
		// webData =
		// "<HTML><HEAD><TITLE>Testing...</TITLE></HEAD><BODY><H1>Testing document...</H1></BODY></HTML>";
		String dataOut = setupViewport(false, webData);
		// System.out.println("setWebContent: injecting: " + dataOut);
		// setupViewport(false, webData);

		adWebView
				.loadDataWithBaseURL(null, dataOut, "text/html", "UTF-8", null);
		// adWebView.loadDataWithBaseURL(null, webData, "text/html", "UTF-8",
		// null);
	}
	...
	synchronized private void setAdContent(AdData ad) {
		...
		// adViewState = MraidInterface.STATES.DEFAULT; already done in ad web
		// view???
	}
	...
	public void update() {
		...
		if ((state == MraidInterface.STATES.DEFAULT)
				|| (state == MraidInterface.STATES.LOADING)) {
			adLog.log(MASTAdLog.LOG_LEVEL_DEBUG, "update", "");
			StartLoadContent();
		} else {
			adLog.log(MASTAdLog.LOG_LEVEL_DEBUG, "update",
					"skipped - state not default (" + state + ")");
		}
	}

	/*
	 * protected void update(boolean isManual) { if (isShown() || isManual) {
	 * adLog.log(MASTAdLog.LOG_LEVEL_ALL, MASTAdLog.LOG_TYPE_INFO, "update",
	 * ""); if (isManual) IsManualUpdate = true; //hideVirtualKeyboard();
	 * StartLoadContent(); } }
	 */

	/**
	 * Set banner refresh interval (in seconds). Once an ad has finished
	 * loading, the timer starts and a new ad will be loaded after this amount
	 * of time has elapsed. Default 120 seconds. If 0, ads are not updated
	 * automatically (use the update() method for a manual update.) If less than
	 * 0, the default value is re-instated.
	 */
	public void setUpdateTime(int updateTime) {
		adLog.log(MASTAdLog.LOG_LEVEL_DEBUG, "setUpdateTime",
	...
	public void StartLoadContent() {
		...
		if ((adserverRequest == null)
				|| (adserverRequest
						.getProperty(MASTAdRequest.parameter_site, 0) == 0)
				|| (adserverRequest
						.getProperty(MASTAdRequest.parameter_zone, 0) == 0)) {
			adReloadTimer.startTimer();
			adLog.log(MASTAdLog.LOG_LEVEL_ERROR, "StartLoadContent",
					"site=0 or zone=0");
			return;
		}
		...
		if ((defaultImageResource != null) && (getBackground() == null)) {
			try {
				handler.post(new SetBackgroundResourceAction(
		...
		if (adWebView.getMraidInterface().getState() != MraidInterface.STATES.EXPANDED) {
			try {
				...
				if (adDelegate != null) {
					MASTAdDelegate.AdDownloadEventHandler downloadHandler = adDelegate
				...
				String url = adserverRequest
						.toString(MASTAdConstants.AD_REQUEST_TYPE_XML);
				lastRequest = url;
				requestCounter++;
				adLog.log(MASTAdLog.LOG_LEVEL_DEBUG,
						"requestGet[" + String.valueOf(requestCounter) + "]",
						url);
				ContentManager.getInstance(this).startLoadContent(this, url);
			} catch (Exception e) {
				adLog.log(MASTAdLog.LOG_LEVEL_ERROR,
						"StartLoadContent.requestGet", e.getMessage());
	...
	private void setErrorResult(AdData ad) {
		if (ad.serverErrorCode != null) {
			...
			if (ad.serverErrorCode == 404) {
				adLog.log(MASTAdLog.LOG_LEVEL_DEBUG, "requestGet result["
						+ String.valueOf(requestCounter) + "][ERROR][CODE="
						+ ad.serverErrorCode + "]", ad.error);
			} else {
				adLog.log(MASTAdLog.LOG_LEVEL_ERROR, "requestGet result["
						+ String.valueOf(requestCounter) + "][ERROR][CODE="
						+ ad.serverErrorCode + "]", ad.error);
			}
		} else {
		...
		if (defaultImageResource != null) {
			try {
				...
				adLog.log(MASTAdLog.LOG_LEVEL_ERROR, "setErrorResult",
						e.getMessage());
			}
		}
		...
		if ((lastResponse != null) && (lastResponse.hasContent())
				&& (lastResponse.error == null) && !isShowPreviousAdOnError) {
			return;
		}
	...
	synchronized public boolean setResult(final AdData ad) {
		if (ad == null) {
			...
			if (this.getParent() == null) {
				...
				adLog.log(MASTAdLog.LOG_LEVEL_DEBUG, "requestGet result["
						+ String.valueOf(requestCounter) + "]", ad.toString());
		...
		if ((ad != null) && (adDelegate != null)
			...
			try {
				...
				if ((ad.externalCampaignProperties != null)
						&& (!ad.externalCampaignProperties.isEmpty())) {
					Iterator<NameValuePair> i = ad.externalCampaignProperties
							.iterator();
					NameValuePair nvp;
					while (i.hasNext()) {
						nvp = i.next();
				...
				adDelegate.getThirdPartyRequestHandler().onThirdPartyEvent(
						(MASTAdView) this, params);
			} catch (Exception e) {
				adLog.log(MASTAdLog.LOG_LEVEL_ERROR, "onThirdPartyRequest",
						e.getMessage());
			}
		}
	}
	...
	public String close(Bundle ignoredBundle) {
		MraidInterface.STATES adState = adWebView.getMraidInterface()
				.getState();
		if (adPlacementType == MraidInterface.PLACEMENT_TYPES.INTERSTITIAL) {
			closeInterstitial();
			if ((customCloseButton != null)
					&& (customCloseButton.getParent() != null)) {
				((ViewGroup) customCloseButton.getParent())
						.removeView(customCloseButton);
			}
			...
			// adWebView.getMraidInterface().fireSizeChangeEvent(this.getWidth(),
			// this.getHeight());
			adWebView.getMraidInterface().fireSizeChangeEvent(resizeToX,
					resizeToY);
			// adWebView.getMraidInterface().fireSizeChangeEvent(AdSizeUtilities.devicePixelToMraidPoint(this.getWidth(),
			// context),
			// AdSizeUtilities.devicePixelToMraidPoint(this.getHeight(),
	...
	public void showInterstitial(int withDuration) {
		if (adPlacementType != MraidInterface.PLACEMENT_TYPES.INTERSTITIAL) {
			adLog.log(MASTAdLog.LOG_LEVEL_DEBUG, "AdViewContainer",
					"Attempt to show interstitial with wrong placement");
			return;
		}
		...
		adSizeUtilities.showInterstitialDialog(showCloseInterstitialTime,
				withDuration);
	}
	...
	public String playVideo(Bundle data) {
		...
		try {
			...
			String error = "Error getting playback uri for video: "
					+ ex.getMessage();
			adLog.log(MASTAdLog.LOG_LEVEL_ERROR, "AdViewContainer.playVideo",
					error);
			return error;
		}
	...
	public String createCalendarEvent(Bundle data) {
		try {
			...
			if (start == null) {
				...
				adLog.log(MASTAdLog.LOG_LEVEL_ERROR,
						"AdViewContainer.createCalendar", error);
				return error;
			}
			...
			return adWebView
					.getMraidInterface()
					.getDeviceFeatures()
					.createCalendarInteractive(description, location, summary,
							start, end);
		} catch (Exception ex) {
			String error = "Error getting parameters for calendar event: "
					+ ex.getMessage();
			adLog.log(MASTAdLog.LOG_LEVEL_ERROR,
					"AdViewContainer.createCalendar", error);
			return error;
	...
	public String open(Bundle data) {
		...
		try {
			...
			// for "action" urls (sms, tel, mailto) just invoke, for others do a
			// fetch/open
	...
	private static final String defaultViewportDefinition = "<meta name=\"viewport\" content=\"width=device-width,initial-scale=1.0,user-scalable=no\"/>";
	...
	private class SetBackgroundResourceAction implements Runnable {
		...
		public void run() {
			try {
				...
				adLog.log(MASTAdLog.LOG_LEVEL_ERROR,
						"SetBackgroundResourceAction", e.getMessage());
			}
		}
	}
	...
	protected void onAttachedToWindow() {
		...
		if (adDelegate != null) {
			MASTAdDelegate.AdActivityEventHandler activityHandler = adDelegate
		...
		if ((lastResponse != null)
				...
				&& ((adWebView == null) || ((adWebView.getMraidInterface()
						.getState() == MraidInterface.STATES.LOADING) || (adWebView
		...
		if ((adWebView != null)
			...
			// adWebView.getTop(), adWebView.getWidth(), adWebView.getHeight());
		}
		...
		if (adDelegate != null) {
			...
			if (activityHandler != null) {
				...
				try {
					...
					adLog.log(MASTAdLog.LOG_LEVEL_ERROR,
							"onAdDetachedFromActivity - exceptioin",
							e.getMessage());
				}
			}
		}
	...
	synchronized public void setLocationDetection(boolean detect,
			Integer minWaitMillis, Float minMoveMeters) {
		if (detect) {
			...
			if (isAccessFineLocation == PackageManager.PERMISSION_GRANTED) {
				locationListener = new AdLocationListener(context,
						minWaitMillis, minMoveMeters,
						LocationManager.GPS_PROVIDER, Looper.getMainLooper(),
						adLog) {
					...
					public void success(Location location) {
						try {
							...
							adserverRequest.setProperty(
									MASTAdRequest.parameter_latitude,
									Double.toString(latitude));

							double longitude = location.getLongitude();
							adserverRequest.setProperty(
									MASTAdRequest.parameter_longitude,
									Double.toString(longitude));

							adLog.log(MASTAdLog.LOG_LEVEL_DEBUG,
									"LocationDetection changed", "(" + latitude
											+ ";" + longitude + ")");
						} catch (Exception e) {
							adLog.log(MASTAdLog.LOG_LEVEL_DEBUG,
									"LocationDetection", e.getMessage());
						}
					}
				};
	...
	public Button createCloseButton(Context c,
	...
	public void onOrientationChange(int orientationAngle, int screenOrientation) {
		...
		if ((adWebView.getMraidInterface().getState() == MraidInterface.STATES.EXPANDED)
			...
			expandedWebView.getMraidInterface().setCurrentPosition(
					coordinates[0], coordinates[1], adWebView.getWidth(),
					adWebView.getHeight());
	...
	public void richmediaEvent(String method, String parameter) {
		if (adDelegate != null) {
			RichmediaEventHandler handler = adDelegate
					.getRichmediaEventHandler();
			if (handler != null) {
