	...
	final private class AdWebViewClient extends WebViewClient {
		...
		public void onPageStarted(WebView view, String url, Bitmap favicon) {
			/* if (supportMraid) {
				...
				if (mraidInterface.getDeviceFeatures().isSupported(
						MraidInterface.FEATURES.INLINE_VIDEO)) {
					if (context instanceof Activity) {

						// XXX do this with reflection??? currently causes a
						// problem if enabled
						final int hardwareAccelerated = 16777216; // WindowManager.LayoutParams.FLAG_HARDWARE_ACCELERATED
																	// from API
																	// level 11
				...
				try {
					...
					screenSize
							.put(MraidInterface
									.get_SCREEN_SIZE_name(MraidInterface.SCREEN_SIZE.WIDTH),
									""
											+ AdSizeUtilities
													.devicePixelToMraidPoint(
															metrics.widthPixels,
															getContext()));
					screenSize
							.put(MraidInterface
									.get_SCREEN_SIZE_name(MraidInterface.SCREEN_SIZE.HEIGHT),
									""
											+ AdSizeUtilities
													.devicePixelToMraidPoint(
															metrics.heightPixels,
															getContext()));
					injectJavaScript("mraid.setScreenSize("
							+ screenSize.toString() + ");");
				} catch (Exception ex) {
				...
				try {
					...
					maxSize.put(
							MraidInterface
									.get_MAX_SIZE_name(MraidInterface.MAX_SIZE.WIDTH),
							""
									+ AdSizeUtilities.devicePixelToMraidPoint(
											metrics.widthPixels, getContext()));
					maxSize.put(
							MraidInterface
									.get_MAX_SIZE_name(MraidInterface.MAX_SIZE.HEIGHT),
							""
									+ AdSizeUtilities.devicePixelToMraidPoint(
											metrics.heightPixels
													- getStatusBarHeight(),
					...
					adLog.log(MASTAdLog.LOG_LEVEL_ERROR, "onPageStarted",
							"Error setting max size information.");
				}
			}
