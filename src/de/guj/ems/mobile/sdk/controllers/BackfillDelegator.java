 ...
 * a backfill ad. A backfill ad contains information on how to delegate
 * the request to an additional SDK such as Smartstream or AdMob.
 * 
 * The response consists of an opening tag naming the backfill partner,
 * any data which will be parsed by an adapter for the backfill partner
 * and a closing tag.
 * 
...
public class BackfillDelegator {
	...
	public static class BackfillData {
		...
		/**
		 * Get the ID identified by the backfill partner in the main adserver response
		 * @return
		 */
		public int getId() {
			return this.id;
	 ...
	 * 
	 * Callbacks must be implemented by classing using the BackfillDelegator to handle
	 * events in the views or activities of an Android App
	...
	public static interface BackfillCallback {
		...
		/**
		 * Called when the ad from the backfill partner has finished (e.g. a movie)
		 */
	...
	/**
	 * Parses the main adserver response for backfill information and returns
	 * an appropriate BackfillData object if something was actually found
	...
	/**
	 * Find the appropriate adapter for a backfill partner and delegates the data
	 * received from the main adserver to it.
	 * Additionally callbacks are triggered upon any events happing during the delegation
	 * @param context Android application context
	 * @param bfData valid BackfillData object with backfill partner ID and data received from the main adserver
	 * @param callback class implementing callback methods so that an application can react to backfill events
	 * @throws BackfillException if any error occurs during triggering of the backfill adapter
	 */
	public final static void process(Context context, BackfillData bfData, BackfillCallback callback) throws BackfillException {
		if (callback != null && bfData != null && bfData.getId() < adapters.length && adapters[bfData.getId()] != null) {
			...
			if (bfData.getId() >= adapters.length || adapters[bfData.getId()] != null) {
				throw new BackfillException("No Adapter found for id=" + bfData.getId() + ", data=" + bfData.getData());
			}
		}
	}
